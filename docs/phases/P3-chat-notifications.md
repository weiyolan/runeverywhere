...
| **Provides to later phases** | `conversations`/`conversation_members`/`messages` + Broadcast-from-DB delivery (P4 live run reuses the private-channel + `realtime.messages` policy pattern); `notifications` + `push_tokens` + `send-push` pipeline (P4 review prompts, P5 badge/leaderboard notifications extend `notification_kind`); `Avatar`/`Badge` components; pg_cron + pg_net enabled; `mark_conversation_read`, `get_or_create_dm` RPCs |
...
5. A third signed-in account that is NOT a member cannot read that conversation, its messages, or subscribe to its Broadcast channel (subscribe errors / receives nothing) — proven by `supabase/tests/rls_chat_smoke.sql`.
...
21. `/dev/components` gallery renders `Avatar` (sizes, initials fallback, ring) and `Badge` (all tones, solid) per their `.d.ts` contracts.
...
| supabase CLI ≥ 2.x with local stack incl. edge runtime | `supabase start` boots; `curl http://127.0.0.1:54321/functions/v1/send-push` reaches the stub |
...
Gotchas the executor must respect: (a) use `uuid_or_null`, never a bare cast (A2); (b) no INSERT policy — clients never `channel.send()`, only the DB broadcasts, so client-write stays denied; (c) authorization is point-in-time at join — a member removed mid-session keeps the socket until unsubscribe/refresh; acceptable because the trigger removal also makes history unreadable (see Risks); (d) P4/P5 add sibling policies for `live:*` topics — keep this one topic-prefixed.

**A10. Seed update** (`supabase/seed.sql`): after the existing fixtures, insert `run_members` rows putting maya (`…0001`) approved into marco's Old Town Loop + Monsanto Hills (triggers then auto-create membership + system messages), a meeting-point message from marco (`'Praça do Comércio · arch · 7:50'`), 3–4 user messages alternating senders, and leave one message unread for maya (set her `last_read_at` back an hour with a direct UPDATE at the end of seeding).

**A11. Commands + acceptance.** `supabase db reset` clean; `psql … -c "select count(*) from conversations"` = 3 (one per seeded run); `select * from list_conversations()` as maya (via `set local role authenticated; set local request.jwt.claims …` per the P1 smoke-test pattern) returns 2 rows with `unread_count > 0` on one.
...
- INSERT with `status='approved'` (open/invite direct join) → notify **host**: `member_joined`, title `<runner> joined <run title>`, body `Your run now has <n> runners going`.
...
- **List**: `FlatList inverted` fed by `useInfiniteQuery(['messages', id])` pages (D2), `onEndReached` loads older. Bubbles: mine = ink bg/white text, right-aligned, radius 15/15/5/15; others = white bg + hairline border, left, avatar+name shown only when the previous (older) message has a different sender; system messages = centered gray pill mapping codes (`joined` → `<name> joined the run`, self → "You joined the run — say hi to the group.", `left`/`removed` accordingly); `meeting_point` renders in-stream as a mini version of the banner. Timestamps small under bubbles.
...
**E1. Port `Avatar` + `Badge`** to `src/components/ui/` from their contracts ([Avatar.d.ts](../../run-everywhere-app-design/project/components/data/Avatar.d.ts): `src/name/size xs–xl/verified/ring`; [Badge.d.ts](../../run-everywhere-app-design/project/components/data/Badge.d.ts): `tone neutral|ink|volt|go|warn|danger|star`, `icon`, `solid`) — initials fallback from `name`, sizes mapped to `sizing` tokens, ring = 2px border in the named token color. Add both to `/dev/components` gallery. (P3 needs them for avatar stacks, unread pills, and notification rows; if P2 already ported either, skip and verify the contract matches.)
...
**F1. `supabase/tests/rls_chat_smoke.sql`** (extends the P1 pattern; `begin…rollback` blocks, role-played JWTs for maya/marco + a third user created in the script): non-member SELECT on `conversations`/`messages` returns 0 rows; non-member message INSERT → RLS error; member INSERT `kind='system'` → check/RLS error; non-host INSERT `kind='meeting_point'` → RLS error; `get_or_create_dm` called both directions returns one id; `notifications` INSERT as authenticated → denied; UPDATE of `title` → permission denied, UPDATE of `read_at` → OK; message fan-out dedupe (B6). Run via `psql postgresql://postgres:postgres@127.0.0.1:54322/postgres -f supabase/tests/rls_chat_smoke.sql`.
...
| File | Creates |
|---|---|
| `supabase/migrations/00000000000030_chat.sql` | Enums `conversation_kind`, `message_kind`; tables `conversations`, `conversation_members`, `messages` (+ indexes, checks per A1); helpers `is_conversation_member`, `uuid_or_null`; RLS per A3; triggers `handle_run_created` (runs INSERT → conversation + host member), `handle_run_member_conversation` (membership sync + system messages), `broadcast_message_inserted` (Realtime broadcast, exception-guarded); RPCs `get_or_create_dm`, `list_conversations`; policy on `realtime.messages` (A9); idempotent backfill for pre-existing runs |
| `supabase/migrations/00000000000031_notifications.sql` | Enum `notification_kind`; tables `notifications` (recipient, kind, title ≤80, body ≤200, run/conversation/actor FKs, read_at, push_sent_at/push_tickets/push_checked_at; reminder-idempotency + message-dedupe partial unique indexes), `push_tokens` (token PK, user, platform); RLS per B2 incl. `grant update (read_at)`; triggers `handle_message_notification` (delete+insert dedupe), `handle_run_member_notification`; RPC `mark_conversation_read` |
| `supabase/migrations/00000000000032_push_pipeline.sql` | Extensions `pg_net`, `pg_cron`; `get_secret` (Vault); trigger `handle_notification_push` (notifications INSERT → `net.http_post` → send-push, skip-if-unconfigured, exception-guarded); `enqueue_run_reminders` + cron `run-reminders` `*/5 * * * *`; `request_push_receipts` + cron `push-receipts` `*/30 * * * *` |

RLS review notes: every new table default-deny with explicit policies; all writes to `conversations`/`conversation_members`/`messages(kind='system')`/`notifications` flow through `security definer` triggers/RPCs (tier 1, PLAN.md §2); the only client INSERTs are `messages` (user/meeting_point, membership-checked) and `push_tokens` (own rows); `notifications` UPDATE is column-restricted to `read_at`; `realtime.messages` gets a topic-scoped SELECT policy only — no client broadcast writes. Trigger functions that touch Realtime or pg_net swallow exceptions so chat/notification writes can never fail on delivery infrastructure. `supabase db lint` + `npm run db:types` after each migration.
...
- **Contracts**: `project/components/data/Avatar.d.ts`, `Badge.d.ts` (ported in E1); existing `TabBar` `messagesBadge` prop; `Input`/`IconButton` from P1.
...
8. Account C (never joined): Explore → cannot open the run chat anywhere; via SQL smoke (F1) C reads 0 conversation/message rows and its forced subscribe to `conversation:<id>` errors/receives nothing.
...
14. `/dev/components` shows Avatar + Badge variants.

## Risks & gotchas

| Risk | Mitigation |
|---|---|
| `realtime.messages` policy raises on cast → **all** private-channel subscribes fail | `uuid_or_null` helper + `like 'conversation:%'` guard (A2/A9); never rely on AND ordering. Test subscribe with a garbage topic in F1 |
...
- **Delete+insert (not upsert) for message-notification dedupe** so the pg_net push trigger stays INSERT-only — one pipeline, no UPDATE special-casing.
...
- **Shared-secret (`x-push-secret`) auth for `send-push`** with `verify_jwt=false`; secret lives in Vault (DB side) + function secrets (Deno side). Service-role key never stored in the database.
...
- Live-run channels, GPS, `complete_run` points, review prompts → **P4** (reuses the private-channel + `realtime.messages` policy pattern established here).
