...
3. **Flags-off sweep:** with all three flags false, the app shows zero integration UI — no "Connected apps" section in `settings/index`, deep-navigating `settings/connections` shows only the empty "Nothing to connect yet." state (no provider rows, C2), no HealthKit write attempt after finishing a run, no IMPORTED cards in the PAST tab, no new permission prompts. Fresh-install QA on both platforms confirms.
...
9. With `strava` ON: CONNECT on the Strava row opens the system browser at Strava's authorize page (scopes `read,activity:read`), approval returns the user to `settings/connections` (auth-session result on iOS, intent deep link on Android — F1/C3), and the row flips to "Connected · syncing runs" with the athlete id in the manage sheet.
...
| Test athlete = the dev's own Strava account with ≥ 1 GPS run activity whose visibility is **not "Only You"** (`activity:read` cannot see private activities — Risks) | strava.com profile shows a run with a map; the activity's visibility is Followers or Everyone |
...
- RLS enabled immediately; single policy: SELECT to `authenticated` `using (true)`. No INSERT/UPDATE/DELETE policies, **plus** `revoke insert, update, delete on public.feature_flags from anon, authenticated;` — the revoke matters: under RLS with no UPDATE policy, a client `update` silently matches zero rows and reports success (`UPDATE 0`); revoking the default grants makes it fail loudly with `permission denied` (asserted in H1). Writes happen as `postgres` (Studio/SQL editor) only.
...
- `useFeatureFlags(): { flags: Record<string, boolean>, isLoading }` — `useQuery({ queryKey: ['flags'], queryFn: () => supabase.from('feature_flags').select('key, enabled'), staleTime: 5 * 60_000 })` (TanStack Query v5 object signature — the repo pins `^5.101.2` and v5 removed the positional overload), mapped to a record. **Default is OFF**: missing row, query error, or loading → `false` (flags-off must be the failure mode). Convenience `useFlag(key: 'healthkit'|'strava'|'garmin'): boolean`.
- Gating convention used by every later task: UI entry points render `null` when the flag is false; imperative paths (post-run HealthKit write) re-check the flag via `queryClient.ensureQueryData` before acting (D5 — the cache alone can be empty on a cold launch). Server-side, `strava-connect`/`garmin-connect` refuse new connections when the flag is off (D3); webhook imports for *already-connected* accounts keep flowing regardless of the flag (Decisions).
...
**B5. `run_tracks` alter (the "small alter" the import needs).** P4 shipped `run_id NOT NULL`, and its Decisions explicitly rejected a nullable FK ("deferred rather than half-supported with a nullable FK") — **P6 knowingly supersedes that decision here, for imported rows only.** P4's rationale was about *app-recorded* runs: the design has no solo-run entry point, so every recorded track has a run. That still holds — the shape check below keeps `source='app'` ⇔ `run_id` present — but an imported activity genuinely has no RE run to reference, a case P4 never weighed:
```sql
...
- `public.import_external_track(p_user_id uuid, p_source public.track_source, p_external_id text, p_title text, p_polyline text, p_distance_m integer, p_duration_s integer, p_elevation_gain_m integer, p_started_at timestamptz, p_ended_at timestamptz) returns uuid` — validates like `complete_run` step 3 (distance 100–200000 m, duration 60–43200 s, elevation 0–10000, `ended_at > started_at`, polyline 1–200000 chars; plus `p_source <> 'app'`); computes `avg_pace_s_per_km = round(p_duration_s / (p_distance_m / 1000.0))`; `insert … on conflict (user_id, source, external_id) where external_id is not null do nothing returning id` — the `where external_id is not null` predicate is mandatory for Postgres to infer the B5 partial unique index (without it: "no unique or exclusion constraint matching the ON CONFLICT specification"). `returning` yields **no row** on conflict, so when the insert returns null, fall back to `select id from public.run_tracks where user_id = p_user_id and source = p_source and external_id = p_external_id` and return that (idempotent replay always yields the track id). On a fresh insert, `update connected_accounts set last_synced_at = now(), last_sync_error = null where user_id = p_user_id and provider = p_source::text::public.integration_provider`.
...
**C3. OAuth return handling.** The callback 302s to `runeverywhere://settings/connections?provider=strava&status=connected|error&reason=…`, and that URL reaches the app by **two different paths**: on iOS, `openAuthSessionAsync` is `ASWebAuthenticationSession` — it intercepts the custom-scheme redirect and surfaces it only in the promise result; the app is never deep-linked and the route never receives the params, so F1 parses `result.url` and feeds this handler directly. On Android (and on any cold-start deep link) the URL arrives as route params, read here via `useLocalSearchParams()`. Both paths converge on one shared handler: `status=connected` → invalidate `['connected-accounts']` + toast "STRAVA CONNECTED"; `status=error` → inline error mapped from `reason` (`denied` → "You declined on Strava — nothing was connected.", `athlete_limit` → "This app is at Strava's 10-athlete test limit.", else generic).

**C4. `settings/index.tsx` (modify P5 F2).** Insert the "CONNECTED APPS" section between ACCOUNT and PREFERENCES (matching the design's order): one row "Connected apps" with sub-line summarizing state ("Strava connected" / "Not connected") → `router.push('/settings/connections')`. Rendered only when at least one **platform-visible** flag is on: `['strava', 'garmin', ...(Platform.OS === 'ios' ? ['healthkit'] : [])]` (`useFeatureFlags`) — mirroring C2's row conditions, so Android with only `healthkit` enabled shows no section rather than linking to a zero-row screen (verification step 3). Acceptance: flags off → section absent (DoD #3); a visible flag on → navigates.
...
**D5. Write-after-complete (modify the P4 finishing sequence in `src/app/live/[runId].tsx` F3).** Insert one step between the raw-samples upload and `buffer.clear()`: if `Platform.OS === 'ios'`, resolve both gates with `queryClient.ensureQueryData` (`['flags']` and `['connected-accounts']`, raced against a short ~3 s timeout; fetch failure or timeout → skip) — never a bare cache read, because on a cold launch straight into recording (deep link, resumed session) neither query may ever have run and an empty cache would silently skip a genuinely connected user's write; flag `healthkit` on and a healthkit row present → `await saveRunToHealthKit({ startedAt, endedAt, distanceM }, samples)` with the same accepted-samples array already in memory for the gzip upload. Success → pass `healthkitSaved: true` via nav params so `recap/[trackId]` shows a one-line "Saved to Apple Health" caption (go-green, under the stat strip); failure → skip silently (no retry queue in v1 — Decisions; the route data needed for a faithful retro-write is gone once the buffer clears). The step must be awaited inside its own try/catch so it can never delay or fail the recap navigation by more than the write itself.
...
**F1. Connect flow (extend C2's Strava row).** CONNECT handler in `src/lib/integrations/strava.ts`: `const { data } = await supabase.functions.invoke('strava-connect')` → `const result = await WebBrowser.openAuthSessionAsync(data.url, 'runeverywhere://settings/connections')`. **Primary return path:** `result.type === 'success'` → parse `result.url`'s query params and hand them to the C3 handler — on iOS the auth session swallows the custom-scheme redirect and delivers it only in this result, never as a deep link. C3's route-param reading covers Android's intent delivery and cold-start deep links as the fallback. `result.type === 'cancel' | 'dismiss'` → quietly reset the row to not-connected. `flag_off`/HTTP 403 → inline "Strava connections are currently disabled." Disconnect: `supabase.functions.invoke('strava-disconnect')` with RPC fallback (C2).
...
**H1. `supabase/tests/rls_integrations_smoke.sql`** (P1 `begin…rollback` + role-played-JWT pattern): as maya — INSERT `connected_accounts (provider='healthkit')` for self OK; for marco's uid → RLS error; INSERT `provider='strava'` → check/RLS error; SELECT returns only own rows; `select public.store_connected_account(…)` and `get_connected_tokens(…)` and `import_external_track(…)` → `permission denied for function`; `update feature_flags set enabled = true` → `permission denied for table feature_flags` (the A2 revoke; a plain RLS-no-policy setup would silently report `UPDATE 0` instead of raising); direct INSERT into `run_tracks` (any source) → RLS error; as postgres — `store_connected_account` twice updates (not duplicates) Vault secrets, `import_external_track` twice → one row, `disconnect_account` (role-played as maya) deletes the row **and** both `vault.secrets` rows (count check); `run_tracks` constraint checks: `source='app'` with null `run_id` → error, `source='strava'` with non-null `run_id` → error. Run: `psql postgresql://postgres:postgres@127.0.0.1:54322/postgres -f supabase/tests/rls_integrations_smoke.sql`.
...
|---|---|
| `supabase/migrations/00000000000060_feature_flags.sql` | `feature_flags` (`key text pk`, `enabled boolean default false`, `note`, `updated_at`); RLS SELECT-only to `authenticated` + write grants revoked from `anon`/`authenticated`; seed rows `healthkit`/`strava`/`garmin` (never overwriting `enabled` on re-push) |
...
| OAuth return delivery differs per platform: iOS `ASWebAuthenticationSession` never deep-links the app — the redirect URL surfaces only in `openAuthSessionAsync`'s resolved value — while Android delivers it as an intent deep link | F1 parses `result.url` from the auth-session result as the primary path (this is what the iOS verify gate exercises); C3's route-param handler is the Android/cold-start fallback; `['connected-accounts']` also refetches on screen focus so a missed toast never means missed state |
...
| Activities set to "Only You" visibility are invisible to `activity:read` — backfill skips them and webhook detail fetches 404, so an all-private athlete imports nothing and verify-gate half 2 fails with no diagnostic | Precondition pins a test run whose visibility isn't "Only You"; the scope deliberately stays minimal `read,activity:read` (Decisions — Extended-Access odds); if private-activity import is ever wanted, `activity:read_all` is a scope-string change plus user re-consent |
...
- **Imports create `run_tracks` rows with `run_id = null`** — P6 supersedes P4's `run_id NOT NULL` decision (its "no nullable FK" rationale addressed solo app runs, which stay excluded — not imports, which have no run to reference) with shape checks (`app` ⇔ has run ⇔ no external_id); idempotency via partial unique `(user_id, source, external_id)`; only GPS activities with a polyline import (P4's `polyline not null` stands; treadmill runs skipped).
