...
| | |
|---|---|
| **Depends on** | P1 (hosted Supabase project linked + pushed, real auth, `database.types.ts`, `Input`/`IconButton`, `supabase/tests/` convention, column grants blocking client writes to `profiles.points_total/level/rating_*`) · P2 (map wrapper component, `run/[id]` detail, Your Runs tabs in `(tabs)/runs`, ported `Tabs`/`Badge`/`Avatar`/`StatBlock` components, migrations `00000000000020`–`29`) · P3 (`notifications` + `notification_kind` + `send-push` pipeline, migrations `00000000000030`–`32`) |
| **Provides to later phases** | `run_tracks`, `points_ledger`, `levels`, `reviews` tables + `complete_run()`/`submit_review()` RPCs; live `profiles.points_total`/`level`/`rating_avg`/`rating_count` caches (P5 leaderboard + badges hang off `points_ledger` inserts and these caches); private `tracks` Storage bucket with raw gzipped GPS (P6 HealthKit/Strava export reads `run_tracks`); the isolated recording module `src/lib/recording/` (P5 live-share reuses its location plumbing); `RatingStars` component (`StatBlock` was ported by P2 — H2 only verifies/extends it); `recap/[trackId]`, `review/[runId]`, `live/[runId]` routes; `notification_kind` values `run_completed` and `review_received` (P5's `notification_pref_key` push gate must map both) |
...
15. Submitting a review updates the reviewee's `profiles.rating_avg`/`rating_count` immediately and inserts a `review_received` notification (+ push) for the reviewee. The fresh aggregate is visible without app restart in P4's one surface for another user's rating — the peer star on `run/[id]`'s RAN WITH rows (H4) — and is cross-checked with a `profiles` query; the full profile display ("Other runners say", `user/[id]`) is P5 (Definition item for "rating aggregates update").
...
20. `/dev/components` gallery renders `RatingStars` (value, partial fill, count, interactive) per its `.d.ts` contract, plus any `StatBlock` variants H2 had to add — the base `StatBlock` gallery section already ships in P2.
...
Rationale: a typical run yields ~90–130 pts (see C4); L2 lands after ~3 runs, L4 after ~12. **Known deviation from the prototype:** the Profile Flow level card reads "1,240 pts · 260 to Tempo" (under the title "Pacer"), implying a next-level threshold of 1,500, while this table puts level 5 at 1,800 — a deliberate P4 decision for an evenly widening ladder. This table is the source of truth: P5 names the levels against it and must **not** re-derive thresholds from the prototype copy. Titles left `''` (P5 names levels). RLS: SELECT to `authenticated` `using (true)`; no writes.
...
3. Validate payload (raise on failure): `p_distance_m between 100 and 200000`; `p_duration_s between 60 and 43200`; `p_elevation_gain_m between 0 and 10000`; `p_ended_at > p_started_at`; `p_started_at > now() - interval '2 days'`; `p_duration_s <= extract(epoch from p_ended_at - p_started_at) + 60` (duration is moving time; must fit inside the wall-clock window); `char_length(p_polyline) between 1 and 200000`; `p_raw_path is null or p_raw_path = (select auth.uid())::text || '/' || p_run_id || '.json.gz'` — the only object the caller's Storage policies allow anyway (B3), so `run_tracks.raw_path` stays a trustworthy pointer for P6's export reads instead of free text.
...
**D1. `reviews` table** per PLAN.md §3: uuid pk, `run_id`, `reviewer_id`, `reviewee_id`, `stars integer check (stars between 1 and 5)`, `tags text[] check` (each element in the design's six: `'Great pace','Welcoming','On time','Knows the city','Strong runner','Good vibes'`, max 6 — enforce with `tags <@ array['Great pace','Welcoming','On time','Knows the city','Strong runner','Good vibes']::text[] and (array_length(tags, 1) is null or array_length(tags, 1) <= 6)` — the parentheses around the length clause are load-bearing: without them `and` binds tighter than `or` and the whitelist collapses to "any 1–6 strings"; I1 includes an invalid-tag rejection case), `note text not null default '' check (char_length(note) <= 200)`, `created_at`; **`unique (run_id, reviewer_id, reviewee_id)`**; `check (reviewer_id <> reviewee_id)`; index `(reviewee_id)`.
...
5. Insert a `notifications` row for the reviewee: kind `'review_received'`, title `<reviewer display_name> || ' rated you'`, body `'New review on ' || <run title>`, `run_id`, `actor_id = caller` — P3's pg_net trigger pushes it automatically. The migration opens with `alter type public.notification_kind add value if not exists 'review_received';` (same in-transaction safety argument as C5); P5's `notification_pref_key` maps this kind to its `reviews` toggle, so the name is a cross-phase contract — do not rename.
6. Return `jsonb_build_object('review_id', …, 'rate_crew_awarded', <bool: did step 4 insert>)` so the UI can show "+10 pts" feedback only the first time.
...
**D6. Seed update** ([supabase/seed.sql](../../supabase/seed.sql)): append a **fifth** fixture run `10000000-0000-4000-8000-000000000005` ("River Loop", social, hosted by marco, `starts_at = now() - interval '2 days'`, `status = 'completed'`) — precondition: P2's seed already occupies `…0004` (invite-only "Track Repeats"), and since all seed inserts are `on conflict (id) do nothing`, reusing that id would silently no-op and attach P4's fixtures to the wrong run — maya approved into it, one `run_tracks` row each for maya + marco (short hand-written encoded polyline near Belém, e.g. 5200 m / 1908 s / 21 m D+), matching `points_ledger` rows (insert directly — the trigger updates caches), and one review maya → marco (5 stars, tags `{'Great pace','Good vibes'}`). Insert ledger/tracks **after** the P3 seed content; keep all inserts idempotent (`on conflict do nothing`).
...
**E1. `src/lib/recording/locationTask.ts`** — `TaskManager.defineTask(LOCATION_TASK, handler)` at **module scope**; export `LOCATION_TASK = 're.live-run'`. Living inside `src/lib/recording/` keeps the workstream-header isolation claim literally true — every expo-location/task-manager touch point sits in the one swappable module. Import this file for its side effect at the top of `src/app/_layout.tsx` (guarantees definition before any headless invocation). The handler: read `{ locations }` from `data`, run each through the filter pipeline (E2), append accepted samples to the buffer (E3), update the meta accumulators, and mirror into the zustand store via `useLiveRun.setState` (module-level import — works whether or not UI is mounted). Errors: log and return (never throw — a throw can unregister the task).
...
- `startRecording(runId)`: `ensureRecordingPermissions()` (A4) → `initSession` → `Location.startLocationUpdatesAsync(LOCATION_TASK, { accuracy: Location.Accuracy.BestForNavigation, activityType: Location.ActivityType.Fitness, timeInterval: 1000, distanceInterval: 5, pausesUpdatesAutomatically: false, showsBackgroundLocationIndicator: true, foregroundService: { notificationTitle: 'Run Everywhere — recording', notificationBody: 'Tracking your run. Tap to return.', notificationColor: '#CCFF00', killServiceOnDestroy: false } })`. (Option names to be re-verified against SDK 56 docs at execution, same caveat as A1.)
...
- FINISH RUN → if `distanceM < 500`: confirm sheet "Not much recorded yet." with KEEP RUNNING / FINISH ANYWAY / DISCARD RUN. FINISH ANYWAY is only offered when the payload can actually pass C4's minimums (`distanceM ≥ 100` **and** moving duration ≥ 60 s); below either floor the sheet shows only KEEP RUNNING / DISCARD RUN — otherwise the RPC is a guaranteed rejection and the retry screen a dead end. Salvage's FINISH NOW (below) is gated by the same minimums. Otherwise straight to finishing.
...
- RPC failure (offline finish): keep the buffer, show retry screen "SAVING FAILED — your run is safe on this phone." with TRY AGAIN / LATER / DISCARD (LATER leaves meta in `salvage` state so E6 re-offers it; DISCARD clears the buffer and returns to `run/[id]` — the escape valve for a payload the server will never accept).

**F4. Entry points (modify P2 screens).**
- `run/[id]`: for host + approved members, when `status = 'published'` and `now() >= starts_at - 30 min`, show primary volt **START RUN** button → A5 explainer (first time) → A4 permissions → `router.push('/live/' + id)`. This **explicitly amends P2's CTA state-machine priority table**: for the host and approved members of a published run, START RUN outranks the `starts_at < now` "This run has already started." banner row (which P2 places above all CTAs); that banner remains for everyone else. The window matches `complete_run`'s earliest-accepted start and has no upper bound while `status = 'published'` (the RPC accepts late starts too); the first finisher's status flip to `completed` ends it, at which point the H4 completed state takes over.
...
**H2. Reuse the P2-ported `StatBlock`** (`src/components/ui/StatBlock.tsx`, ported in P2 C4 with a gallery section from P2 C9 — do **not** port it again): verify it still matches [StatBlock.d.ts](../../run-everywhere-app-design/project/components/data/StatBlock.d.ts) (`value, label, unit?, accent?, align='center', size sm|md|lg`); extend it only if a size/accent/align variant that F2/G2/H4 need is missing, and add any such new variant to the existing gallery section. Add `RatingStars` (all H1 states) to `/dev/components`.
...
**H4. `run/[id]` completed state (modify P2 screen).** When `status = 'completed'`: render the Reward Loop "COMPLETED RUN DETAIL" layout — map header with the viewer's static track polyline (fallback: start-point pin) + ink COMPLETED pill; type chip + date; title/area/goal; YOUR RESULT `StatBlock` strip (only when the viewer has a track); points-earned ink card `+<sum> pts earned` with caption "Finishing · distance goal · on time · reviews"; "RAN WITH · <n>" rows: `Avatar`, name with the peer's live aggregate inline star (`profiles.rating_avg`/`rating_count`, the P2 host-card star pattern — P4's only surface that renders another user's aggregate; DoD 15 and verification step 10 lean on it), plus the viewer's rate status (`RatingStars` of your given rating, or RATE chip → `/review/[runId]`); footer DONE → back. Join/manage affordances hidden for completed runs.
...
**H6. `send-push` deep-link map (modify [supabase/functions/send-push/index.ts](../../supabase/functions/send-push/index.ts)).** Add `run_completed → '/review/' + run_id` and `review_received → '/run/' + run_id` (the completed detail — P4 has no received-reviews surface) to `deepLinkFor`, and both kinds → `'reminders'` in `channelFor` (reuses P3's existing Android channel — no new channel needed); redeploy `supabase functions deploy send-push --no-verify-jwt`. Acceptance: tapping the completion push cold-starts into `review/[runId]`; tapping a review push opens the completed `run/[id]`.

### I — Verification pass (always last)

**I1. `supabase/tests/rls_points_smoke.sql`** (P1/P3 pattern: `begin…rollback` blocks, role-played JWTs for maya/marco + a third non-member): `complete_run` twice → second returns `already_completed`, ledger count unchanged; non-participant `complete_run` → error; `complete_run` on a cancelled run → error; direct INSERT into `run_tracks`/`points_ledger`/`reviews` as authenticated → RLS error; UPDATE `profiles.points_total`/`rating_avg` as authenticated → permission denied (P1 grants); `submit_review` self / non-completed / non-participant reviewee → errors; a tag outside the six-tag whitelist (e.g. `'{Speedy}'`) or 7 tags → check-constraint error (proves D1's parenthesized `tags` check); same reviewee twice → `'already reviewed'`; two different reviewees → two review rows, **one** `rate_crew` ledger row, and one `review_received` notification per reviewee; `profiles.rating_avg/rating_count` reflect inserts; storage: cross-user object insert into `tracks` → RLS error. Run via `psql postgresql://postgres:postgres@127.0.0.1:54322/postgres -f supabase/tests/rls_points_smoke.sql`.

**I2. Field-test protocol + Transistorsoft risk gate.** Before declaring the phase done, log ≥ 3 outdoor runs per platform in `docs/field-tests.md` (date, device, OS, km app vs. reference, D+, screen-locked minutes, anomalies). **Exit criteria to stay on expo-location**: distance error ≤ 10 % on every run, zero recordings that died with the screen locked, no polyline gaps > 60 s. Any failure → adopt the escape hatch (PLAN.md §1/§6): buy Transistorsoft `react-native-background-geolocation`, reimplement only `src/lib/recording/recorder.ts` + `src/lib/recording/locationTask.ts` (both inside the isolated module) against its API (store/UI contracts unchanged), rebuild, re-run the protocol. Budget: 1 week + license cost.
...
| `supabase/migrations/00000000000042_reviews.sql` | `alter type notification_kind add value if not exists 'review_received'`; `reviews` (uuid pk, `run_id`/`reviewer_id`/`reviewee_id` FKs, `stars 1–5`, `tags text[]` constrained to the six design tags (parenthesized check, D1), `note ≤ 200`, **`unique (run_id, reviewer_id, reviewee_id)`**, `reviewer <> reviewee`, index `(reviewee_id)`), RLS SELECT reviewer-or-reviewee, no client writes; RPC `submit_review` (D3, incl. the once-per-run `rate_crew` ledger insert + `review_received` fan-out); trigger `apply_review_rating` (AFTER INSERT → full recompute of `profiles.rating_avg`/`rating_count`, definer); RPC `list_past_runs` (invoker, D5) |
...
- **Contracts**: [RatingStars.d.ts](../../run-everywhere-app-design/project/components/data/RatingStars.d.ts) (ported H1), [StatBlock.d.ts](../../run-everywhere-app-design/project/components/data/StatBlock.d.ts) (P2 port, verified H2); `Avatar`/`Badge`/`Tabs` (P2), `Button`/`TypeChip` (P0), `Input`/`IconButton` (P1).
...
8. Idempotency: from A, re-invoke `complete_run` with the same payload — primary path: a temporary dev button (runs under A's real JWT); SQL-editor alternative: the call **must** be wrapped in the P1 smoke role-play boilerplate (`begin; set local role authenticated; select set_config('request.jwt.claims', '{"sub":"<A-uid>","role":"authenticated"}', true); …; rollback;`) because a bare editor session has `auth.uid() = null` and fails C4's auth guard instead of replaying. Expect `already_completed: true`; `select count(*) from points_ledger where run_id = …` unchanged; `profiles.points_total` unchanged. This is the verify-gate "points idempotent" check.
...
10. Reviews: A rates B 5★ + tags + note → "+10 PTS EARNED" toast; B receives the `review_received` push (deep link → completed `run/[id]`); on A's completed run detail, B's RAN WITH row now shows the peer aggregate star `5.0 (1)` after refetch, **without restart** (H4 — the surface DoD 15 names); cross-check `select rating_avg, rating_count from profiles where id = <B>` → `5.00 / 1` — verify-gate "rating aggregates update". A rates a second crew member → review saved, no second +10 (ledger count for `rate_crew` = 1). A tries the same reviewee again → friendly "already reviewed".
...
| Risk | Mitigation |
|---|---|
| Background GPS dies (OEM task killers, iOS suspension) — PLAN.md §6 top risk | Foreground service + persistent notification (A1/E5); bounded user-started session; buffer survives kills (E3/E6); **field-test protocol with explicit exit criteria and the paid Transistorsoft swap confined to `src/lib/recording/` (`recorder.ts` + `locationTask.ts`)** (I2) |
...
- **Levels defaults:** 10 levels, 0→8000 pts (C1 table) — deliberately deviates from the prototype level card's implied 1,500 next-level threshold ("1,240 pts · 260 to Tempo"); the C1 table is the threshold source of truth P5's level names must honor; titles empty until P5.
